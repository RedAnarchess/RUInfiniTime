...
Steps::Steps(Controllers::MotionController& motionController, Controllers::Settings& settingsController)
  ...
  lv_label_set_text_static(lstepsL, "Шаги");
  lv_obj_align(lstepsL, lSteps, LV_ALIGN_OUT_BOTTOM_MID, 0, 5);

  lv_obj_t* lstepsGoal = lv_label_create(lv_scr_act(), nullptr);
  lv_obj_set_style_local_text_color(lstepsGoal, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_CYAN);
  lv_label_set_text_fmt(lstepsGoal, "Цель: %5lu", settingsController.GetStepsGoal());
  ...
  lv_label_set_text_static(resetButtonLabel, "Сбросить");
  ...
  lv_label_set_text_fmt(tripLabel, "Поездка: %5li", currentTripSteps);
...
void Steps::Refresh() {
  ...
  if (currentTripSteps < 100000) {
    ...
    lv_label_set_text_fmt(tripLabel, "Поездка: 99999+");
  }
  lv_arc_set_value(stepsArc, int16_t(500 * stepsCount / settingsController.GetStepsGoal()));
}
